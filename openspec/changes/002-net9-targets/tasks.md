...
- [x] 1.1 Add `net9.0` to `<TargetFrameworks>` in `src/Here.Sdk.Common/Here.Sdk.Common.csproj`.
...
- [x] 1.2 Add `net9.0` to the CI matrix in `.github/workflows/ci.yml`.
  - **Acceptance:** CI matrix includes `net9.0` in the build + test strategy.
  - **Verification:** CI workflow green on PR.

- [x] 1.3 Update `global.json` to accept .NET 9 SDK alongside .NET 8 (rollForward policy).
