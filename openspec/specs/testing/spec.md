...
Defines test organization, coverage gates (90 % line/branch, 95 % method per file and
globally), determinism rules, integration test skip conventions, mutation testing baseline,
and snapshot testing policy for `Here.Sdk.Common`.
...
- **WHEN** a credentialed `[SkippableFact]` runs without `HERE_ACCESS_KEY_ID` set
...
### Requirement: Test organization and naming

The system SHALL organize tests under `tests/Here.Sdk.Common.UnitTests/` with one test class per source class, named `Method_Condition_ExpectedOutcome` for every method.
