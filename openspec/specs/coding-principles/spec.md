...
Defines the language, clean-code, architecture, and API-style standards that every
`Here.Sdk.*` source file must meet, enforced via analyzers, pre-commit hooks, and CI.
...
- **WHEN** a PR introduces a non-ASCII identifier such as `src/Foo/Données.cs`
- **THEN** the non-ASCII identifier CI step exits non-zero AND the PR is blocked

#### Scenario: Common typo blocks commit

- **WHEN** a file staged for commit contains a word codespell identifies as a misspelling
