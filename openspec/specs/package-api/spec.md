...
Defines every public type exposed by `Here.Sdk.Common`: geography value objects
(`GeoCoordinates`, `GeoBearing`, `GeoBoundingBox`, `GeoPolyline`), units, error hierarchy,
geometry, localization, identifiers, and the full set of domain enums (transport, routing,
search, navigation, traffic, positioning, EV, map).
...
### Requirement: `GeoBearing` normalized to `[0, 360)`

The system SHALL provide `public readonly record struct GeoBearing(double DegreesFromNorth)` that normalizes input to `[0, 360)` in the primary constructor.
...
#### Scenario: Equality across equivalent normalized inputs
...
- **THEN** every specialized subclass is caught
...
- `public readonly record struct GeoOrientation(double? BearingInDegrees, double? TiltInDegrees, double? RollInDegrees)` — all values nullable; if present, Bearing normalized to `[0, 360)`, Tilt clamped to `[-90, 90]`, Roll to `[-180, 180]`.
...
- `public readonly record struct Anchor2D(double HorizontalOffset, double VerticalOffset)` — offsets in `[0, 1]` range representing normalized 2D coordinates.
...
### Requirement: Localization value objects

The system SHALL provide under `Here.Sdk.Common.Localization`:
